Feature: notifying respondent to verify their email address
  ...
  Scenario: Unverified account user is notified to verify their email address
    Given an external "unverified1@email.com" with unverified account tried to sign into their account
  ...
  Scenario: Verifying a users account
    ...
    When they select the verification link in the email "unverified1@email.com"
