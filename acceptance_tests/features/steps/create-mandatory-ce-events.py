...
@given("the internal user has created a new collection exercise for '{survey}' and period '{period}' with shown as '{shown_period}'")
...
def assert_redirect_to_ce_details_page(_):
    assert browser.find_by_summary("Collection exercise details")


@then("the new mandatory event date of '{day}' '{month}' '{year}' is displayed")
def assert_new_date_displayed(_, day, month, year):
    assert day in browser.find_by_name("mps-date").text
    assert month in browser.find_by_name("mps-date").text
